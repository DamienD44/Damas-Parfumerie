CREATE TABLE IF NOT EXISTS `damasparfum`.`user` (
...
INSERT INTO `damasparfum`.`user` (`name`, `email`, `password`) VALUES
...
CREATE TABLE IF NOT EXISTS `damasparfum`.`parfums` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  `user_id` INT,
  PRIMARY KEY (`id`),
  FOREIGN KEY (`user_id`) REFERENCES `damasparfum`.`user` (`id`) ON DELETE SET NULL
) ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;

INSERT INTO `damasparfum`.`parfums` (`name`, `marque`, `description`, `image`, `user_id`) VALUES
